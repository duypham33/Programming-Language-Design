...
class Operators:
    def __init__(self, scoperule):
        #Scoping rule (static versus dynamic)
    ...
    """
       Helper function. Pushes a tuple of the given scope_entry and an empty dictionary onto the dictstack. 
    """   
    def dictPush(self, scope_entry):
        self.dictstack.append((scope_entry, {}))
    ...
    def define(self, name, value):
        if len(self.dictstack) == 0:
          self.dictstack.append((0, {}))  #Push the initial dict, 0 will be the first link index
    ...
    def lookup(self, name):
        name = '/' + name
        cur_index = len(self.dictstack) - 1
        
        while cur_index >= 0:
            d = self.dictstack[cur_index][1]
            if name in d:
              return cur_index, d[name]
            
            if self.scope == 'dynamic':
              cur_index -= 1
            elif self.scope == 'static':
              static_link = self.dictstack[cur_index][0]
              cur_index = static_link if cur_index > 0 else -1
            else:
              break
    ...
    def stack(self):
      print("===**opstack**===")
      for i in range(len(self.opstack) - 1, -1, -1):
        print(self.opstack[i])
      
      print("===**dictstack**===")
      for i in range(len(self.dictstack) - 1, -1, -1):
        print(f"----{i}----{self.dictstack[i][0]}----")
        d = self.dictstack[i][1]
    ...
    def psIf(self):
        if len(self.opstack) > 1:
          if isinstance(self.opstack[-1], FunctionValue) == False:
            ...
            if condition == True:
              #Get the most recent environment entry
              end = len(self.dictstack) - 1
              code.apply(self, end)  #Execute
        else:
    ...
    def psIfelse(self):
        if len(self.opstack) > 2:
          if isinstance(self.opstack[-1], FunctionValue) == False:
            ...
            if condition == True:
              code1.apply(self, end)
            else:
              code2.apply(self, end)
        else:
