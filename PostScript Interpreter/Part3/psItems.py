...
class Array(Expr):
    ...
    def evaluate(self, psstacks):
        ...
        arr_value = ArrayValue(lst)
        psstacks.opPush(arr_value)
        return arr_value
...
class Name(Expr):
    ...
    def evaluate(self, psstacks):
        if self.var_name[0] == '/':
            ...
            scope_entry, value = psstacks.lookup(self.var_name)
            if isinstance(value, FunctionValue):
                value.apply(psstacks, scope_entry)  #Execute the method
            else:
...
class Block(Expr):
    ...
    def evaluate(self, psstacks):
        funct_value = FunctionValue(self.value)
        psstacks.opPush(funct_value)
        return funct_value
...
class FunctionValue(Value):
    ...
    def apply(self, psstacks, scope_entry):
        #Add Activation Record
        psstacks.dictPush(scope_entry)

        #Execute
        for expr in self.body:
            expr.evaluate(psstacks)

        #Pop the dictionary from the stack
        psstacks.dictPop()
